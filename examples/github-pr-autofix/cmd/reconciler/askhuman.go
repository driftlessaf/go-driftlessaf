...
// askHumanToolName is the held-out suspend tool advertised to the model when the
// ENABLE_ASK_HUMAN demo path is active. The model calls it to pause the run for a
// human answer; the executor then returns a *checkpoint.Suspension.
const askHumanToolName = "ask_human"

// askHumanToolDescription is the human-facing description advertised for the
// suspend tool.
const askHumanToolDescription = "Pause the run and ask a human operator a question. " +
	"Provide the question text; the run resumes once a human answers."

// askHumanCheckpointPrefix is the object-name prefix for parked envelopes
// when CHECKPOINT_BUCKET wires the durable GCS checkpoint store.
const askHumanCheckpointPrefix = "github-pr-autofix"
...
// with the parked conversation and the human's answer silently dropped, while
// a workqueue retry re-reconciles and re-runs from scratch.
var errResumeFailed = errors.New("ask-human: resuming claimed run failed")

// prAgentFactory constructs a fresh meta-agent. The ask-human resume path builds
...
// askHumanReconciler carries the suspend/resume lifecycle wiring for the
// ENABLE_ASK_HUMAN demo. With CHECKPOINT_BUCKET set, the run is durable and
// the human transport is GitHub itself: envelopes park in GCS (gcsstore) and
...
// a restart.
type askHumanReconciler struct {
	coord    *suspend.Coordinator
	newAgent prAgentFactory
}

// newAskHumanReconciler wires a checkpoint store and question store into a
...
func newAskHumanReconciler(ctx context.Context, cfg *config, clients *githubreconciler.ClientCache) (*askHumanReconciler, error) {
	if !strings.HasPrefix(strings.ToLower(cfg.Model), "claude-") {
		return nil, fmt.Errorf("ENABLE_ASK_HUMAN requires a claude-* model (suspend/resume is only wired for the Claude backend today), got AGENT_MODEL=%q", cfg.Model)
	}
	...
	if cfg.AskHumanBucket != "" {
		...
		store = gcsstore.New(askHumanCheckpointPrefix, client.Bucket(cfg.AskHumanBucket), gcsstore.IdentitySealer{})
		questions = githubquestions.New(clients)
	} else {
		js, err := jsonlstore.New(cfg.AskHumanSnapshotPath)
		if err != nil {
	...
	coord, err := suspend.New(store, questions, cfg.AskHumanWakeInterval)
	if err != nil {
	...
	// parked.
	coord.Jitter = cfg.AskHumanWakeInterval / 2
	return &askHumanReconciler{
		coord:    coord,
		newAgent: newAskHumanAgentFactory(cfg),
	}, nil
}

// newAskHumanAgentFactory returns a factory that builds a fresh meta-agent with
// the ask-human suspend tool advertised. The suspend tool is opt-in via the
// additive metaagent.Config fields; the agent is otherwise identical to the
// plain autofix agent.
func newAskHumanAgentFactory(cfg *config) prAgentFactory {
	return func(ctx context.Context) (metaagent.Agent[*PRContext, *PRFixResult, PRTools], error) {
		return metaagent.New[*PRContext, *PRFixResult, PRTools]( //nolint:infertypeargs // Req cannot be inferred from Config
			...
			metaagent.Config[*PRFixResult, PRTools]{
				...
				SuspendToolName:        askHumanToolName,
				SuspendToolDescription: askHumanToolDescription,
			},
		)
	}
}

// run drives one reconcile of the ask-human lifecycle for key. It is the demo's
...
//     agent from scratch. If the model calls the ask-human tool, Execute
...
//     run would otherwise be marked failed by its own polls before a human
//     could answer.
...
func (r *askHumanReconciler) run(ctx context.Context, key string, req *PRContext, cb PRTools, beginFresh func(context.Context) error) (*PRFixResult, error) {
	decision, res, err := r.coord.Wake(ctx, key)
	if err != nil {
		return nil, fmt.Errorf("ask-human wake: %w", err)
	}

	switch decision {
	case suspend.WakeRearm:
		// Parked and waiting on a human: requeue cheaply, touch nothing, and burn
		// no fix attempt. This is the poll wake that must never re-execute.
		clog.InfoContextf(ctx, "ask-human: %s still awaiting an answer, rearming", key)
		return nil, workqueue.RequeueAfterWithJitter(r.coord.WakeInterval, r.coord.Jitter)

	case suspend.WakeResume:
		clog.InfoContextf(ctx, "ask-human: %s answered, resuming", key)
		agent, err := r.newAgent(ctx)
		if err != nil {
			return nil, fmt.Errorf("ask-human: building resume agent: %w", err)
		}
		resumer, ok := metaagent.AsResumer[*PRContext, *PRFixResult, PRTools](agent)
		if !ok {
			return nil, errors.New("ask-human: agent backend does not support resume")
		}
		...
		// call the ask-human tool again. Re-park exactly like the fresh
		...
		if s, ok := checkpoint.AsSuspension(err); ok {
			clog.InfoContextf(ctx, "ask-human: %s suspended again at turn %d, re-parking", key, s.Turn)
	...
	default: // WakeFresh
		...
		agent, err := r.newAgent(ctx)
		if err != nil {
			return nil, fmt.Errorf("ask-human: building fresh agent: %w", err)
		}
		result, err := agent.Execute(ctx, req, cb)
		if s, ok := checkpoint.AsSuspension(err); ok {
			// The model asked a human. Park the checkpoint and post the question;
			// Suspend returns a workqueue requeue that pauses the key.
			clog.InfoContextf(ctx, "ask-human: %s suspended at turn %d, parking", key, s.Turn)
			return nil, r.coord.Suspend(ctx, key, s)
		}
		return result, err
	}
...
// answersForPending maps each pending tool-call ID to the raw human answer
