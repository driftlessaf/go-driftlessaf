...
// The core state-machine vocabulary (Status, FailureMode, the generic
// Trigger constants, and StateTransition) lives in the shared statemachine
// package so every metareconciler flavor emits the same contract. The
// aliases below keep this package's public API unchanged for existing
// consumers; see the statemachine package for the full documentation.

// Status is a Linear-issue-driven reconciler's progress state on an issue.
// Alias of [statemachine.Status].
type Status = statemachine.Status

// Framework-defined Status values. See the statemachine package docs.
const (
	StatusActive   = statemachine.StatusActive
	StatusComplete = statemachine.StatusComplete
	StatusFailed   = statemachine.StatusFailed
)

// FailureMode classifies why a State landed on StatusFailed.
// Alias of [statemachine.FailureMode].
type FailureMode = statemachine.FailureMode

// FailureMode values persisted alongside StatusFailed on the state
// attachment. See the statemachine package docs.
const (
	FailureModeMaxTurns   = statemachine.FailureModeMaxTurns
	FailureModePRClosed   = statemachine.FailureModePRClosed
	FailureModeNoDiff     = statemachine.FailureModeNoDiff
	FailureModeNoProgress = statemachine.FailureModeNoProgress
)

// Generic Trigger values live in the statemachine package (see the
// MAINTAINER NOTE there for the agent-engagement vs framework-observation
// classification bots depend on). Aliased here so existing consumers keep
// compiling; the two Linear-workflow-specific triggers below stay local.
const (
	TriggerInitialRun               = statemachine.TriggerInitialRun
	TriggerCIFailureIteration       = statemachine.TriggerCIFailureIteration
	TriggerDescriptionEditIteration = statemachine.TriggerDescriptionEditIteration
	TriggerMergeConflict            = statemachine.TriggerMergeConflict
	TriggerMaxTurns                 = statemachine.TriggerMaxTurns
	TriggerPRMerge                  = statemachine.TriggerPRMerge
	TriggerPRClosed                 = statemachine.TriggerPRClosed
	TriggerNoDiff                   = statemachine.TriggerNoDiff
	TriggerNoProgress               = statemachine.TriggerNoProgress
)
...
// Alias of [statemachine.StateTransition].
type StateTransition = statemachine.StateTransition
