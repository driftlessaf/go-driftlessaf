...
// one system prompt; with none set it behaves like WithSystemInstructions.
...
// prompt (such as the judge's JSON output format) applies it with this option
// last, where a caller's WithSystemInstructions can add to it but not replace
// it. Neither prompt is modified.
